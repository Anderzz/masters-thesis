...
def convert_to_nnunet_format(
    ...
    train_val_loc = os.path.join(
        nnunet_raw_loc, f"Dataset{dataset_id_str}_CAMUS_trainval"
    )
    nnunet_dataset_id += 1
    dataset_id_str = str(nnunet_dataset_id).zfill(3)
...
>>>>>>> 317660c589d3d6eef5dbd70999913b9615366e24
                    if patient in train_set or patient in val_set:
                        ...
                        us_png.save(
                            os.path.join(
                                train_val_loc, images_tr_loc_trainval, save_name_us
                            )
                        )
                        gt_png.save(
                            os.path.join(
                                train_val_loc, labels_tr_loc_trainval, save_name_gt
                            )
                        )
                    elif patient in test_set:
                        # save to test folder
                        us_png.save(
                            os.path.join(test_loc, images_tr_loc_test, save_name_us)
                        )
                        gt_png.save(
                            os.path.join(test_loc, labels_tr_loc_test, save_name_gt)
                        )
